...
void initializeTrafficCUDA(CarV3* &cars, int* &numCarsInLanes, int* &carIndicesInLanes) {
    ...
    for (int carIdx=0; carIdx<NUM_CARS; ++carIdx) {
        ...
        pos_idx = indices[carIdx]%LANE_LENGTH;
        // fill a car according to lane_idx and pos_idx, assigning random speed up to SPEED_LIMIT
        numCarsInLanes[lane_idx]++;
        carIndicesInLanes[lane_idx*LANE_LENGTH + numCarsInLanes[lane_idx] - 1] = carIdx; // Link carIdx to the last element in corresponding lane_idx of carIndicesInLanes matrix
        cars[carIdx].Position = pos_idx;
        cars[carIdx].Speed = (rand()%SPEED_LIMIT) + 1;
        cars[carIdx].TargetSpeed = cars[carIdx].Speed;
    }
    for (int laneIdx = 0; laneIdx < NUM_LANES; ++laneIdx) {
        ...
        // sort the temp array
        std::sort(tempCarIndices, tempCarIndices + numCarsInLanes[laneIdx], [&cars](int a, int b) {
            return cars[a].Position < cars[b].Position;
        });
...
void printStepCarsCUDA(FILE* &fid, CarV3* &cars, int* &numCarsInLanesDevice, int* &carIndicesInLanesDevice) {
    for (int carIdx = 0; carIdx < NUM_CARS; ++carIdx) {
        ...
        bool carFound = false;
        for (int laneIdx = 0; laneIdx < NUM_LANES; ++laneIdx) {
            for (int j=0; j<numCarsInLanesDevice[laneIdx]; ++j) {
                if (carIdx == carIndicesInLanesDevice[laneIdx*LANE_LENGTH + j]) {
                    fprintf(fid, "%d,%d", laneIdx, cars[carIdx].Position);
                    carFound = true;
                    break;
                }
